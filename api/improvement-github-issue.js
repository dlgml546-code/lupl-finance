import { readJsonBody, requireManager, send } from "./_shared.js";

function issueBody(requests) {
  const items = requests.map((request, index) => [
    `## ${index + 1}. ${request.menu || "페이지 미지정"}${request.submenu ? ` / ${request.submenu}` : ""}`,
    `- 유형: ${request.type || "-"}`,
    `- 상태: ${request.status || "-"}`,
  ...
  return ["경영대시보드 개선함에서 생성된 이슈입니다.", "", `요청 수: ${requests.length}건`, "", ...items].join("\n\n");
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const repo = process.env.LUPL_GITHUB_REPO || "dlgml546-code/lupl-finance";
    ...
    const labels = String(process.env.LUPL_GITHUB_ISSUE_LABELS || "improvement").split(",").map((label) => label.trim()).filter(Boolean);
    const payload = { title, body: issueBody(requests), labels };
